                ...
                <div className="space-y-4">
                    ...
                    <section>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <InfoItem
                                label="Total Funding"
                                value={company.props.company_financials_highlights.funding_total
                    ...
                    <section>
                        <h3 className="font-semibold mb-2">Recent Key Employee Changes</h3>
                        <ul className="space-y-2">
                            {company.props.key_employee_change_list.slice(0, 3).map((change) => (
                                <li key={change.identifier.uuid}>
                                    <p>{change.press_reference_publisher} - {new Date(change.key_event_date).toLocaleDateString()}</p>
                                    <a href={change.press_reference_link.value} className="text-blue-600 hover:underline" target="_blank" rel="noopener noreferrer">
                    ...
                    <section>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {company.props.current_advisors_image_list.map((advisor) => (
                                <Avatar key={advisor.identifier.uuid} title={advisor.person_identifier.value}>
                                    <AvatarImage src={`https://images.crunchbase.com/image/upload/c_pad,h_45,w_45,f_auto,b_white,q_auto:eco,dpr_4/${advisor.person_identifier.image_id}`} alt={advisor.person_identifier.value} />
                                    <AvatarFallback>{advisor.person_identifier.value.slice(0, 2).toUpperCase()}</AvatarFallback>
                                </Avatar>
                            ))}
                        </div>
                    </section>
                    ...
                    <section>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {company.props.investors_list.filter(investor => investor.is_lead_investor).slice(0, 5).map((investor) => (
                                <Badge key={investor.identifier.uuid} variant="secondary">
