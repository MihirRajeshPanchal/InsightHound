...
export default function AudienceSegment() {
    ...
    const { data } = useQuery({
        ...
        queryFn: async () => {
            const response = await fetchAPI<{ segments: Segment[] }, TNoParams, { id: string }>({
                url: "/marketsegment",
                method: "POST",
    ...
    return (
        <div className="flex flex-col p-4">
            ...
            <Select onValueChange={(str) => setSelectedSegment(Number(str))}>
                <SelectTrigger>{data.segments[selectedSegment].segment}</SelectTrigger>
                <SelectContent>
                    {data?.segments.map((seg, idx) => (
                        <SelectItem value={String(idx)} key={idx}>{seg.segment}</SelectItem>
                    ))}
                </SelectContent>
            </Select>
