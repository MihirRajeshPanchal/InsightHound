                        ...
                        <div key={index} className="mb-4">
                            {message.role === 'ai' ? (
                                <div className="bg-muted w-4/5 p-3 rounded-lg">
                                    {message.content ?
                                        <div dangerouslySetInnerHTML={{ __html: message.content }} className="whitespace-pre-wrap" />
                                        : "Cooking..."}
                                </div>
                            ) : (
            ...
            <CardFooter>
                <form onSubmit={handleSubmit} className="flex flex-col w-full flex-1">
                    <ScrollArea className="pb-2 w-full flex *:*:flex *:*:gap-2">
                        ...
                        {suggestions.map((suggestion, index) => (<Button disabled={isLoading} key={index} type="submit" onClick={() => setInput(suggestion)} variant="outline">{suggestion}</Button>))}
                    </ScrollArea>
                    <div className='flex space-x-2'>
                        ...
                        />
                        <Button type="submit"><Send className='rotate-45 mr-2' /></Button>
                    </div>
                </form>
            </CardFooter>
        </Card>
