	...
	return image ? (
		<div>
			<h2 className="text-xl font-semibold pb-4">
				Image Upload and Text Placement
			</h2>
			<Input
				className="w-fit"
				type="file"
			...
			<div className="flex w-full justify-between items-center">
				...
				<div>
					...
					<div className="flex flex-wrap gap-4 pt-4">
						{customFontNames.map((font, index) => (
							<button
								key={index}
								onClick={() => handleFontRemoval(index)}
								className="p-2 bg-foreground text-background rounded-lg w-fit text-xs flex items-center"
							>
								{font} <Trash size={14} />
							</button>
						))}
			...
			<div className="flex gap-6">
				<div ref={imageRef} className="relative h-full w-1/2 mt-5">
				...
				<ScrollArea
					...
					style={{ height: imageRef.current?.clientHeight }}
				>
