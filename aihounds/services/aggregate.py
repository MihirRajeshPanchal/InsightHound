...
from aihounds.services.outreach import (
    generate_linkedin,
    generate_rivals,
...
def convert_to_json_string(data):
    ...
    try:
        ...
        return json.dumps(
            {"error": "Unable to convert data to JSON", "original_data": str(data)}
        )
...
def do_aggregate(
    ...
    if last_ai_response and last_ai_response.get("action") == "response_md_pending":
        if last_ai_response.get("tool_call_id"):
            ...
            tool_calls = [
                create_tool_call(name=tool_name, id=last_ai_response["tool_call_id"])
            ]
            ai_msg = AIMessage(
                content=last_ai_response.get("data", ""), tool_calls=tool_calls
            )
...
def generate_conversation_title_from_query(query: str):
    chain = (
        ...
        | OutputFixingParser.from_llm(
            parser=JsonOutputParser(pydantic_object=GenerateTitleResponse),
            llm=openai_llm,
        )
    )
