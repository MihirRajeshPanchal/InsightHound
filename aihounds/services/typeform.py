...
def create_typeform_definition(survey_request: SurveyRequest) -> dict:
    ...
    return {
        ...
        "theme": {"href": "https://api.typeform.com/themes/default"},
        "workspace": {"href": f"https://api.typeform.com/workspaces/{WORKSPACE_ID}"},
        "settings": {
            "language": "en",
        ...
        "fields": [
            {
                ...
                "properties": {
                    "choices": [
                        {"label": option, "ref": f"choice_{index}_{opt_index}"}
...
def generate_typeform(vision, mission, description, domain):
    """
    Generates a form survey or questionnaire based on the provided vision, mission, description, and domain only if asked to.
    ...
    survey_request["questions"] = generate_questionnaire(
        vision, mission, description, domain
    )
...
async def generate_typeform_responses(questionnaire_data, form_id):
    ...
    """
    question_mapping = {
        str(idx): question for idx, question in enumerate(questionnaire_data)
    }
    ...
    async with httpx.AsyncClient() as client:
        ...
        for item in typeform_responses.get("items", []):
            for answer in item.get("answers", []):
                question_ref = str(
                    list(question_refs.keys()).index(answer["field"].get("ref"))
                )
        ...
        for ref, frequencies in option_frequencies.items():
            ...
            response_frequencies.append(
                ResponseFrequency(
                    question_text=question_details.get(
                        "questionText", f"Question {ref}"
                    ),
